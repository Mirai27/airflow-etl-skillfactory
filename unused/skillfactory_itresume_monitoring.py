...
def extract_and_load_raw(**kwargs):
    # Получаем дату исполнения (логическую дату)
    execution_date = kwargs['ds']
    dt = datetime.strptime(execution_date, '%Y-%m-%d')
    
    # Вычисляем границы недели
    start_week = (dt - timedelta(days=dt.weekday())).strftime('%Y-%m-%d')
    end_week = (dt + timedelta(days=6 - dt.weekday())).strftime('%Y-%m-%d')
...
def export_to_csv(**kwargs):
    ...
    file_name = f"reports/agg_results_{USER_ID}_{kwargs['ds']}.csv"
...
# --- DAG ---
with DAG(
    'skillfactory_itresume_monitoring',
    default_args=default_args,
    description='Weekly API to Postgres ETL with idempotency',
    schedule='@weekly',
    catchup=False
) as dag:
